...
func (a *GormOutboxAdapter) Save(ctx context.Context, rec *OutboxRecord) error {
...
func (a *GormOutboxAdapter) ListPending(ctx context.Context, limit int) ([]*OutboxRecord, error) {
	...
	for _, r := range rows {
		out = append(out, &OutboxRecord{ID: uuid.MustParse(r.ID), Aggregate: r.Aggregate, Type: r.Type, Payload: r.Payload, CreatedAt: r.CreatedAt, Published: r.Published, PublishedAt: r.PublishedAt, Attempts: r.Attempts, LastError: r.LastError})
	}
	return out, nil
}
