...
func NewLedgerService(w WalletRepository, l LedgerRepository) *LedgerService { return &LedgerService{wallets: w, ledger: l} }
...
func (s *LedgerService) Debit(ctx context.Context, walletID string, amount Amount, desc string) error {
	w, err := s.wallets.Get(ctx, walletID)
	if err != nil { return err }
	if err := w.Debit(amount); err != nil { return err }
